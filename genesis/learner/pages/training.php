...
$questionIdParam = intval($_GET['questionId'] ?? 0);
$answerStatus = $_GET['answer'] ?? '';
...
if ($levelId == 1) {
    ...
    $prevStmt = $connect->prepare("SELECT Complete FROM learnerlevel WHERE LearnerId=? AND LevelId=?");
...
if($questionIdParam > 0){
    ...
    $stmtQ = $connect->prepare("
        ...
        WHERE pq.GradeName=? AND pq.SubjectName=? AND pq.Chapter=? AND pq.LevelId=? 
...
<div class="box box-solid" style="border-top:3px solid #605ca8; margin-bottom:10px;">
    <div class="box-header with-border" style="background-color:#f3edff;">
        <h3 class="box-title" style="color:#605ca8;"><i class="fa fa-folder-open"></i> Question Details</h3>
    </div>
    <div class="box-body">
...
<h3 class="box-title" style="color:#3c8dbc;"><i class="fa fa-question-circle"></i> Question</h3>
</div>
...
<div class="radio">
<label>
<input type="radio" name="answer" value="<?= htmlspecialchars($key) ?>" required> <?= htmlspecialchars($val) ?>
</label>
</div>
<?php endforeach; ?>
...
<div class="box box-primary" style="border-top:3px solid #3c8dbc;">
<div class="box-header with-border" style="background-color:#f0f8ff;">
<h3 class="box-title" style="color:#3c8dbc;"><i class="fa fa-bar-chart"></i> Practice Info</h3>
</div>
<div class="box-body" style="background-color:#ffffff;">
...
// SweetAlert after submission
const answerStatus = "<?= $answerStatus ?>";
if(answerStatus === 'correct'){
    Swal.fire({ icon:'success', title:'Correct!', text:'Well done! ✅', confirmButtonText:'OK' });
} else if(answerStatus === 'wrong'){
    Swal.fire({ icon:'error', title:'Incorrect!', text:'Try the next question. ❌', confirmButtonText:'OK' });
}
</script>
