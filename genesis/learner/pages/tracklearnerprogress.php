        ...
        <div class="container-fluid">
            ...
            <?php if (!empty($subjects)): ?>
                <?php foreach ($subjects as $subject): ?>
                    <?php
                    $learnerSubjectId = $subject['LearnerSubjectId'] ?? 0;
                    $startLevel = intval($subject['CurrentLevel'] ?? 1);
                    $targetLevel = intval($subject['TargetLevel'] ?? 7);
                    ...
                    $denominator = $targetLevel - $startLevel;
                    $progress = $denominator > 0 ? (($currentLevel - $startLevel) / $denominator * 100) : 100;
                    ...
                    // Status text
                    if ($averageMark < 50) {
                        $statusText = 'Bad: Immediate intervention recommended.';
                    } elseif ($averageMark < 80) {
                        $statusText = 'Average: Learner is improving.';
                    } else {
                        $statusText = 'Excellent: Keep up the momentum.';
                    }
                    ?>
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <div class="alert alert-warning">No subjects assigned yet. Please contact your tutor.</div>
            <?php endif; ?>
...
function updateSkill(learnerSubjectId, score) {
    ...
    tracker.innerHTML = '<h5>Goal Tracker</h5><p>' + statusText + '</p>';
}
</script>
