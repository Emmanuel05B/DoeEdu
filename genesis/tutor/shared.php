...
$dsql = "SELECT * FROM details WHERE LearnerId = $learner_id AND CreatedAt = (SELECT MAX(CreatedAt) FROM details WHERE LearnerId = $learner_id)";
$dresults = $connect->query($dsql);
...
$sql_present = "SELECT COUNT(*) AS present_count FROM welcome WHERE LearnerId = $learner_id AND Attendance IN ('Present', 'Late');";
$result_present = $connect->query($sql_present);
$present = $result_present->fetch_assoc();

// Query to get absent count
$sql_absent = "SELECT COUNT(*) AS absent_count FROM welcome WHERE LearnerId = $learner_id AND Attendance = 'Absent';";
...
$sql = "SELECT ClassroomTransition FROM welcome WHERE LearnerId = $learner_id ORDER BY Id DESC LIMIT 5;";
$results = $connect->query($sql);
// Initialize an array to hold the results
$transitions = [];

if ($results) {
    $transitions = $results->fetch_all(MYSQLI_ASSOC);
}

$d1 = $transitions[0]['ClassroomTransition'];
$d2 = $transitions[1]['ClassroomTransition'];
$d3 = $transitions[2]['ClassroomTransition'];
$d4 = $transitions[3]['ClassroomTransition'];
$d5 = $transitions[4]['ClassroomTransition'];
///////////////////////////////////////////////////////averages
