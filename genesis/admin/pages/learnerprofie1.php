  ...
  <?php include_once(ADMIN_PATH . "/../partials/mainsidebar.php"); ?>
    ...
    <div class="content-wrapper">
          ...
          <?php
              ...
              if(isset($_GET['id'])){
                $learnerId = $_GET['id'];   // from the url.
...
<?php
$learnerId = $_SESSION['UserId']; // adjust if your session uses a different key

$query = $connect->prepare("
    SELECT s.SubjectName, ls.StartLevel, ls.CurrentLevel, ls.TargetLevel, ls.AverageMark, ls.AttendanceRate, ls.ExpectedLevel
...
while ($goal = $result->fetch_assoc()):
    $progress = ($goal['CurrentLevel'] - $goal['StartLevel']) / ($goal['TargetLevel'] - $goal['StartLevel']) * 100;
?>
    <div class="profile-personal-info">
        <div class="profile-skills border-bottom mb-4 pb-2">
            ...
            <div class="bubble-container row" style="margin-bottom: 15px;">
                <div class="bubble col-md-2">Start Level: <span class="label label-primary"><?= $goal['StartLevel'] ?></span></div>
                <div class="bubble col-md-2">Current Level: <span class="label label-warning"><?= $goal['CurrentLevel'] ?></span></div>
                <div class="bubble col-md-2">Target Level: <span class="label label-success"><?= $goal['TargetLevel'] ?></span></div>
                <div class="bubble col-md-2">Average Mark: <span class="label label-danger"><?= $goal['AverageMark'] ?>%</span></div>
                <div class="bubble col-md-2">Attendance Rate: <span class="label label-default"><?= $goal['AttendanceRate'] ?>%</span></div>
            </div>
            ...
            <div class="progress" style="height: 20px;">
                <div class="progress-bar progress-bar-info progress-bar-striped active"
                    ...
                    Level <?= $goal['CurrentLevel'] ?> of <?= $goal['TargetLevel'] ?>
                </div>
            </div>

            <div class="callout callout-info" style="margin-top: 20px;">
                <h5>Goal Tracker</h5>
                <p>Based on current average, learner is expected to reach Level <?= $goal['ExpectedLevel'] ?> by term end.</p>
            </div>
        </div>
    </div>
<?php endwhile; ?>
