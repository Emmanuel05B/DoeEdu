...
$grade     = isset($_GET['gra']) ? $_GET['gra'] : $activity['Grade'];
$subjectId = isset($_GET['sub']) ? intval($_GET['sub']) : $activity['SubjectId'];
$group     = isset($_GET['group']) ? $_GET['group'] : $activity['GroupName'];

// Get ClassID for this grade, subject, and group
$classStmt = $connect->prepare("SELECT ClassID FROM classes WHERE Grade = ? AND SubjectId = ? AND GroupName = ? LIMIT 1");
$classStmt->bind_param("iis", $grade, $subjectId, $group);
$classStmt->execute();
...
if ($classRow = $classResult->fetch_assoc()) {
    ...
    die("<h3 class='text-center text-danger'>Class not found for Grade $grade, Subject $subjectId, Group $group.</h3>");
}
$classStmt->close();

// Get due date and assigned date for this activity/class
$assignedAt = null;
$dueDate = null;
                ...
                <form id="feedbackForm" action="emailsuperhandler.php" method="post">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 text-right" style="padding-top: 10px;">
                            <?php if (!empty($activity['LastFeedbackSent'])): ?>
                                <div style="color: #555;">
                                    <i class="fa fa-calendar"></i> Last feedback sent on:
                                    <strong><?= date('d M Y, H:i', strtotime($activity['LastFeedbackSent'])) ?></strong>
                                </div>
                            <?php else: ?>
                                <div style="color: #777;">
