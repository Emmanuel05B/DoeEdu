...
if (isset($_POST['btnsendT'])) {
    ...
    $query = "SELECT Id FROM employee";  //WHERE employeeType = 1;
    ...
    if (!empty($IDs)) {
        ...
        $query2 = "SELECT Email, Name FROM users WHERE Id IN ($in)";
        ...
        echo '<script>
                Swal.fire({
                    icon: "success",
                    title: "Announcement successfully sent to all the Teachers!",
                    confirmButtonColor: "#3085d6",
                    confirmButtonText: "OK"
                }).then((result) => {
                    if (result.isConfirmed) {
